...
import { Condition, MongoClient, MongoClientOptions, ObjectId } from 'mongodb';
...
export const verifyUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (result) {
            if (
                CryptoJS.AES.decrypt(result.password, 'hello').toString(
                    CryptoJS.enc.Utf8
                ) ===
                CryptoJS.AES.decrypt(req.body.password, 'hello').toString(
                    CryptoJS.enc.Utf8
                )
            ) {
                // is the password ok?
...
export const getUserProfile = async (req: Request, res: Response) => {
    ...
    try {
        const result = await db.collection('users').findOne(
            { _id: req.params.id as unknown as Condition<ObjectId> },
...
export const getAllCategories = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log(
            `got allCategories.categories array length: ${allCategories.categories.length}`
        );

        const allHealthCategories = allCategories.categories
            .filter(
                (category: Category) => category.parent_aliases[0] === 'health'
            )
...
export const getProvidersForCategory = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const response = await axios.get(
            `https://api.yelp.com/v3/businesses/search?categories=${req.body.category}&location=${req.body.postalcode}`,
            {
                headers: YelpApiHeader,
            }
        );
        ...
        console.log(
            `got providers.businesses array length: ${providers.businesses.length}`
        );
...
export const getMessageById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (result) {
            ...
            if (
                req.body.requestingId === result.recipientId ||
...
export const getAppointmentById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (result) {
            ...
            if (
                req.body.requestingId === result.patientId ||
...
export const getPatients = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const patients = [
            ...new Map(
                senders.map((item) => [item['senderId'], item])
            ).values(),
        ];
